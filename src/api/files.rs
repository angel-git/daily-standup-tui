...
impl Files {
    pub fn new() -> Files {
        let mut files = Files {
            ...
            config_app: FileConfig {
                devs: DevelopersConfig {
                    names: Vec::from(["Example Dev 1", "Example Dev 2"])
                        .into_iter()
                        .map(String::from)
                        .collect(),
                },
                settings: Some(SettingsConfig { seconds: 180 }),
            },
        };
    ...
    fn write_toml_file(&self) {
        let toml_string =
            toml::to_string(&self.config_app).expect("Can't parse toml default values");
